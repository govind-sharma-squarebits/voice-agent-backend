...
const STEP_CONFIG: Record<Exclude<LeadStep, LeadStep.COMPLETE>, StepConfig> = {
  [LeadStep.NAME]: {
...
export class LeadCollector {
   ...
   */
  getQuestion(step: LeadStep): string {
    if (step === LeadStep.COMPLETE) return '';
   ...
   */
  getNextStep(step: LeadStep): LeadStep {
    if (step === LeadStep.COMPLETE) return LeadStep.COMPLETE;
   ...
   */
  isComplete(step: LeadStep): boolean {
   ...
   */
  validate(step: LeadStep, value: string): { valid: boolean; error?: string } {
    if (step === LeadStep.COMPLETE) return { valid: true };
    ...
    const schema = leadStepSchemas[step as Exclude<LeadStep, LeadStep.COMPLETE>];
   ...
   */
  getRetryMessage(step: LeadStep, error: string): string {
    const retries: Partial<Record<LeadStep, string>> = {
