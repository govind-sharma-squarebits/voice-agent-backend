import { MessageDto } from '../types/index.js';
...
export class ConversationMemoryManager {
  ...
  private totalTokens(messages: MessageDto[]): number {
   ...
   */
  trimHistory(history: MessageDto[]): MessageDto[] {
    let trimmed = [...history];
   ...
   */
  evictStaleSessions<T extends { lastActivityAt: Date }>(
    sessions: Map<string, T>,
    inactiveThresholdMs: number,
    onEvict: (sessionId: string, session: T) => Promise<void>,
  ): void {
    const now = Date.now();
    const toEvict: Array<[string, T]> = [];
   ...
   */
  isAtCapacity(sessions: Map<string, unknown>): boolean {
