...
import { MessageRole, AppError } from '../types/index.js';
...
export class AIService {
    ...
    async withRetry(fn, attempts = 3, delayMs = 500) {
            ...
            const result = await this.withRetry(async () => {
                ...
                return Promise.race([
                    ...
                    new Promise((_, reject) => {
                        signal.addEventListener('abort', () => reject(new AppError('AI_TIMEOUT', 'Gemini request timed out')));
                    }),
                ]);
            }, 3, 500);
    ...
    addToHistory(role, content) {
        ...
        const mappedRole = role === MessageRole.USER ? 'user' : 'model';
        const history = this.chatSession['_history'];
        if (history) {
            history.push({ role: mappedRole, parts: [{ text: content }] });
            if (history.length > 40)
                history.splice(0, 2); // trim pairs
        }
    }
    getSystemPrompt() {
