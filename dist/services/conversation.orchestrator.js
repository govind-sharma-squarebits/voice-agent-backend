import { AgentMode, AppError, LeadStep, MessageRole, TeqviraService, } from '../types/index.js';
...
export class ConversationOrchestrator {
    ...
    sessions = new Map();
    evictionTimer = null;
    constructor(aiService, voiceService, memoryManager, leadCollector, conversationRepo, leadRepo) {
    ...
    // ── Session Lifecycle ──────────────────────────────────────────
    async handleSessionInit(sessionId, socket) {
    ...
    async handleTextMessage(sessionId, content, socket) {
        const session = this.sessions.get(sessionId);
        if (!session) {
    ...
    // ── Lead Collection ────────────────────────────────────────────
    async handleLeadStep(sessionId, value, socket) {
        const session = this.sessions.get(sessionId);
        if (!session || session.leadStep === null)
            return;
        ...
        if (currentStep !== LeadStep.COMPANY || (processedValue && processedValue.toLowerCase().trim() !== 'skip')) {
            session.leadData[currentStep] = processedValue.trim();
        }
    ...
    async completeLead(sessionId, socket, session) {
        try {
            const dto = {
                ...session.leadData,
                sessionId,
            ...
            socket.emit('lead:complete', { leadId: lead._id?.toString(), message: completion });
    ...
    // ── Audio Handling ─────────────────────────────────────────────
    async handleAudioChunk(sessionId, chunk, socket) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
        session.lastActivityAt = new Date();
    ...
    async handleAudioTurnEnd(sessionId, socket) {
    ...
    async streamResponse(text, socket, sessionId) {
    ...
    async streamAIResponse(userText, socket, sessionId, session) {
    ...
    async persistPendingHistory(sessionId, session) {
    ...
    startEvictionCycle() {
        this.evictionTimer = setInterval(() => {
            this.memoryManager.evictStaleSessions(this.sessions, config.SESSION_EVICTION_MS, async (sessionId, session) => {
                await this.persistPendingHistory(sessionId, session);
                this.voiceService.cleanup(sessionId);
            });
